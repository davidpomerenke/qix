...
export function renderGame(ctx: CanvasRenderingContext2D, state: GameState, config: GameConfig) {
  ...
  // Fill claimed area (outside inner border, inside outer border)
  // Use composite operation: fill outer, then cut out inner
  if (state.innerBorder.length >= 3) {
    ...
    // Draw inner border as hole (counter-clockwise)
  ...
  if (state.fuse.active) {
    ...
    ctx.beginPath()
    ctx.arc(state.fuse.pos.x, state.fuse.pos.y, 5, 0, Math.PI * 2)
    ctx.fill()
    ctx.shadowBlur = 0
  }
  ...
  for (const sparx of state.sparx) {
    ...
    ctx.beginPath()
    ctx.arc(sparx.pos.x, sparx.pos.y, sparx.isSuper ? 6 : 4, 0, Math.PI * 2)
    ctx.fill()
    ctx.shadowBlur = 0
  }
...
function renderQix(ctx: CanvasRenderingContext2D, qix: Qix) {
